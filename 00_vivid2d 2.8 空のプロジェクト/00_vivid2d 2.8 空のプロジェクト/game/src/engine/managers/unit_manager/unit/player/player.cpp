...
#include "../../../../../component/collider_component/mesh_collider_component/mesh_collider_component.h"
...
void CPlayer::Move(void)
{
    if (m_Parent != nullptr)
    {
        ...
        auto meshComp = m_Parent->GetComponent<MeshColliderComponent>();
        for (int i = 0; i < 9; i++)
        {
            ...
            CVector3 hitPos = end;
            meshComp->CheckHitLine(start, end, hitPos);
            if (hitPos != end)
            {
        ...
        if (releaseFlag || !meshComp->IsEnabled())
            m_Parent = nullptr;
    }
    else
    {
    ...
    std::shared_ptr<CGameObject> floorObject = CObjectManager::GetInstance().CheckHitObject(shared_from_this());
