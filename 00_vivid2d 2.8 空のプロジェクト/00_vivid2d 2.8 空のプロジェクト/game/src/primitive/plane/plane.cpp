...
void CPlane::UpdateTransform()
{
	...
	m_WorldMatrix = mat_scale * mat_rot * mat_trans;
}

void CPlane::Draw() const
{
	//行列から頂点座標を計算
	...
	//左上、右上、左下、右下の順番
	vertices[0].pos = CVector3(-width_half * m_Transform.scale.x, -height_half * m_Transform.scale.y, 0.0f) * m_WorldMatrix;
	vertices[0].norm = m_Normal * DxLib::MGetRotElem(m_WorldMatrix);
	vertices[0].u = 0.0f;
	vertices[0].v = 1.0f;
	vertices[0].dif = m_Color;
	vertices[0].spc = m_Color;

	vertices[1].pos = CVector3(width_half * m_Transform.scale.x, -height_half * m_Transform.scale.y, 0.0f) * m_WorldMatrix;
	...
	vertices[2].pos = CVector3(width_half * m_Transform.scale.x, height_half * m_Transform.scale.y, 0.0f) * m_WorldMatrix;
	...
	vertices[3].pos = CVector3(-width_half * m_Transform.scale.x, height_half * m_Transform.scale.y, 0.0f) * m_WorldMatrix;
